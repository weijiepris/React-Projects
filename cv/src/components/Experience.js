import React from "react";
import classes from "./css/AboutMe.module.css";

const Experience = () => {
  return (
    <div className={classes.container}>
      <div className={classes.content}>
        <div className={classes.header}>MindChamps</div>
        <p>
          March 2014 - January 2020
          <ul>
            <li>Assisting trainers in classroom activities</li>
            <li>Ensure students are behaving in class</li>
            <li>Guide pre-school students phonics, reading and writing</li>
          </ul>
        </p>
        <br />
        <br />
        <hr />
        <br />
        <br />
        <div className={classes.header}>NCS Pte Ltd</div>
        <p>
          October 2016 - December 2016
          <ul>
            <li>Design and main website using HTML, CSS &amp; Javascript</li>
            <li>Migration of old intranet systems to new intranet systems</li>
            <li>
              Perform User Acceptance Test (UAT) for government intranet
              systems.
            </li>
            <li>
              Help client’s query their needs based on their requirements.
            </li>
            <li>
              Enhance and implement new aesthetics and features of client’s
              current website &amp; systems.
            </li>
          </ul>
        </p>
        <br />
        <br />
        <hr />
        <br />
        <br />
        <div className={classes.header}>Micro 2000 Ptd Ltd</div>
        <br />
        <p>
          October 2013 - December 2013
          <ul>
            <li>
              Interact with customers to find out the issue with their Apple
              devices
            </li>
            <li>
              Running diagnosis on their devices and explaining the procedures
            </li>
            <li>
              Allowed me to work on my soft skills. (communication, confidence
              in speaking)
            </li>
          </ul>
        </p>
        <br />
        <br />
        <hr />
        <br />
        <br />
        <div className={classes.header}>Micro 2000 Ptd Ltd</div>
        <br />
        <p>
          November 2011 - April 2012
          <ul>
            <li>Installation of computer hardware</li>
            <li>Installation of windows 7 image onto desktops and laptops </li>
            <li>Provisioning all machines to meet policy requirements</li>
            <li>
              Deployed to work in most of the Primary and Secondary schools in
              Singapore
            </li>
            <li>Setting up connection to the SSOE VPN</li>
            <li>
              Guiding the staffs and teachers in the school on how to use the
              new system
            </li>
          </ul>
        </p>
      </div>
      <br />
      <br />
      <br />
      <br />
      <br />
      <br />
      <br />
      <br />
    </div>
  );
};

export default Experience;
